  ...
  <div class="conf-step__wrapper">
    ...
    <ul class="conf-step__selectors-box">
      ...
      <li wire:click="selectHall({{ $hall->id }})" wire:key="{{ $hall->id }}">
        <input type="radio" class="conf-step__radio" name="chairs-hall" value="{{ $hall->id }}" wire:model="currentHall">
    ...
    <div class="conf-step__legend">
      <label class="conf-step__label">Рядов, шт<input type="text" class="conf-step__input" wire:model.blur="rows_count.{{ $hall->id }}"></label>
      <span class="multiplier">x</span>
      <label class="conf-step__label">Мест, шт<input type="text" class="conf-step__input" wire:model.blur="seats_count.{{ $hall->id }}"></label>
        ...
        @for ($i = 1; $i <= $rows_count[$hall->id]; $i++)
          <div class="conf-step__row">
            @for ($j = 1; $j <= $seats_count[$hall->id]; $j++)
              <span class="conf-step__chair conf-step__chair_{{ $seats[$hall->id][$i][$j] }}" wire:click="switchSeatType({{ $i }}, {{ $j }})" wire:key="{{$hall->id}}-{{$i}}-{{$j}}"></span>
              @endfor
          </div>
          @endfor
          @endif
